...
function render() {
  ...
  $("#tokens").innerHTML = section.tokens.map((token, index) => {
    ...
    const known = lexemeIds.some((id) => seenLexemes.has(id));
    lexemeIds.forEach((id) => seenLexemes.add(id));
    const firstLexeme = lexemeIds.length ? map.get(lexemeIds[0]) : null;
...
function showToken(index, sourceButton = null) {
  ...
  if (!isPhrase) {
    $("#detail-lemma").textContent = primaryLexeme?.lemma || "—";
    $("#detail-type").textContent = primaryLexeme?.type || "—";
    $("#detail-morphology").textContent = primaryLexeme?.morphology || "—";
    $("#detail-root").textContent = primaryLexeme?.root || "—";
  ...
  if (isRepeat) {
    $("#repeat-note").textContent = token.reference_note || "Lafaz ini menggunakan kembali identitas leksikal yang sudah dianalisis sebelumnya.";
  }
